...
// and Conjugate of complex number input array
        fprintf(outputf, "%lf\n", COMPLEX::getReal(myCarray[j], form));
        fprintf(outputf, "%lf\n", COMPLEX::getImaginary(myCarray[j], form));
        fprintf(outputf, "%lf\n", COMPLEX::getMagnitude(myCarray[j], form));
        fprintf(outputf, "%lf\n", COMPLEX::getPhase(myCarray[j], form));

        ptemp = COMPLEX::getPower(nPower, myCarray[j], form);
        rtemp = COMPLEX::getRoot(mRoot, myCarray[j], form);
        ctemp = COMPLEX::getConjugate(myCarray[j]);
...
// Addition, Subtraction, Multiplication, Division with Second Number entered by user
        atemp = COMPLEX::add(myCarray[j], secondnumber, form);
        stemp = COMPLEX::subtract(myCarray[j], secondnumber, form);
        mtemp = COMPLEX::multiply(myCarray[j], secondnumber, form);
        dtemp = COMPLEX::divide(myCarray[j], secondnumber, form);
